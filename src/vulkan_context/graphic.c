...
enum SSP_ERROR_CODE ssp_vulkan_create_descriptor_set_layout(struct SSPVulkanContextExtFunc *ext_func, VkDevice logical_device, VkDescriptorSetLayout *descriptor_set_layout)
{
    ...
    if (ext_func->vkCreateDescriptorSetLayout(logical_device, &descriptor_info, NULL, descriptor_set_layout) != VK_SUCCESS)
    ...
    VkDevice logical_device)
{
    ...
    for (size_t i = 0; i < SSP_MAX_FRAMES_IN_FLIGHT; ++i) {
        ...
        ext_func->vkUpdateDescriptorSets(logical_device, 1, &write_descriptor, 0, NULL);
    }
}
    ...
    VkBuffer *uniform_buffers)
{
    ...
    *descriptor_sets = calloc(SSP_MAX_FRAMES_IN_FLIGHT, sizeof(VkDescriptorSet));
    ...
    if (ext_func->vkAllocateDescriptorSets(logical_device, &descriptor_set_info, *descriptor_sets) != VK_SUCCESS) {
        free(layouts);
        return false;
    }

    ssp_vulkan_update_descriptor_sets(ext_func, uniform_buffers, *descriptor_sets, logical_device);
    ...
    VkDescriptorPool *descriptor_pool)
{
    ...
    if (ext_func->vkCreateDescriptorPool(logical_device, &descriptor_info, NULL, descriptor_pool) != VK_SUCCESS)
...
{
    *uniform_buffers = calloc(SSP_MAX_FRAMES_IN_FLIGHT, sizeof(VkBuffer));
    *uniform_buffers_memory = calloc(SSP_MAX_FRAMES_IN_FLIGHT, sizeof(VkDeviceMemory));
    *uniform_buffers_mapped = calloc(SSP_MAX_FRAMES_IN_FLIGHT, sizeof(void *));

    for (size_t i = 0; i < SSP_MAX_FRAMES_IN_FLIGHT; ++i) {
        VkDeviceSize size = sizeof(struct SSPShaderUniformBuffer);

        if (ssp_vulkan_create_buffer(ext_func, logical_device, physical_device, size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &((*uniform_buffers)[i]), &((*uniform_buffers_memory)[i])) != SSP_ERROR_CODE_SUCCESS
            || ext_func->vkMapMemory(logical_device, (*uniform_buffers_memory)[i], 0, size, 0, &((*uniform_buffers_mapped)[i])) != VK_SUCCESS)
    ...
    VkDevice logical_device)
{
    ...
    *swapchain_image_views = calloc(swapchain_images_count, sizeof(VkImageView));
    for (int i = 0; i < swapchain_images_count; ++i) {
        create_info.image = swapchain_images[i];
        if (ext_func->vkCreateImageView(logical_device, &create_info, NULL, &((*swapchain_image_views)[i])) != VK_SUCCESS)
    ...
    VkPipeline *graphic_pipeline)
{
    ...
    VkShaderModule shader_module = ssp_vulkan_create_shader_module(ext_func, logical_device, shader_code, code_size);
    ...
    *viewport = (VkViewport) {
    ...
    if (ext_func->vkCreatePipelineLayout(logical_device, &pipeline_layout_info, NULL, pipeline_layout) != VK_SUCCESS) {
        ext_func->vkDestroyShaderModule(logical_device, shader_module, NULL);
    ...
    VkResult result = ext_func->vkCreateGraphicsPipelines(logical_device, NULL, 1, &graphic_pipeline_info, NULL, graphic_pipeline);
...
void ssp_vulkan_update_view(struct SSPCamera *camera, void **uniform_buffers_mapped)
{
    ...
    for (size_t i = 0; i <  SSP_MAX_FRAMES_IN_FLIGHT; ++i)
        memcpy(uniform_buffers_mapped[i], &view, sizeof(mat4));
}
...
void ssp_vulkan_update_proj(struct SSPCamera *camera, void **uniform_buffers_mapped, VkExtent2D swapchain_extent)
{
    ...
    for (size_t i = 0; i <  SSP_MAX_FRAMES_IN_FLIGHT; ++i)
        memcpy(PTR_OFFSET(uniform_buffers_mapped[i], sizeof(mat4)), &proj, sizeof(mat4));
}
...
{
    ext_func->vkResetCommandBuffer(graphic_command_buffers[current_frame], VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
    VkCommandBufferBeginInfo begin_info = {0};
    begin_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;

    ext_func->vkBeginCommandBuffer(graphic_command_buffers[current_frame], &begin_info);
    ssp_vulkan_transition_image_layout(ext_func, &((*swapchain_images)[image_index]), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, 0, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT, graphic_command_buffers[current_frame], VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED);
    ...
    ext_func->vkCmdBeginRenderingKHR(graphic_command_buffers[current_frame], &rendering_info);
    
    ext_func->vkCmdBindPipeline(graphic_command_buffers[current_frame], VK_PIPELINE_BIND_POINT_GRAPHICS, graphic_pipeline);
    ext_func->vkCmdSetViewport(graphic_command_buffers[current_frame], 0, 1, &viewport);
    
    VkRect2D scissor = {0};
    scissor.extent = swapchain_extent;

    ext_func->vkCmdSetScissor(graphic_command_buffers[current_frame], 0, 1, &scissor);
    ext_func->vkCmdBindDescriptorSets(graphic_command_buffers[current_frame], VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_layout, 0, 1, &((*descriptor_sets)[current_frame]), 0, NULL);
    ...
    for (size_t i = 0; i < objects_to_draw->size; ++i) {
        ...
        ext_func->vkCmdBindVertexBuffers(graphic_command_buffers[current_frame], 0, 1, &object->vertex_buffer, &offset);
        ext_func->vkCmdBindIndexBuffer(graphic_command_buffers[current_frame], object->index_buffer, offset, VK_INDEX_TYPE_UINT16);
        ext_func->vkCmdPushConstants(graphic_command_buffers[current_frame], pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(struct SSPShaderPushConstant), &object->vertex_push_constant);
        ext_func->vkCmdDrawIndexed(graphic_command_buffers[current_frame], object->indices_count, 1, 0, 0, 0);
    }
    objects_to_draw->size = 0;

    ext_func->vkCmdEndRenderingKHR(graphic_command_buffers[current_frame]);
    ssp_vulkan_transition_image_layout(ext_func, &((*swapchain_images)[image_index]), VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_ACCESS_2_COLOR_ATTACHMENT_WRITE_BIT, 0, VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT, graphic_command_buffers[current_frame], VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED);
    ext_func->vkEndCommandBuffer(graphic_command_buffers[current_frame]);
